...
class WutongFace(object):
    ...
    def detect(self, image, min_face_size=20.0,
        ...
        bounding_boxes, landmarks = detect_faces(image, min_face_size=50, thresholds=thresholds,nms_thresholds=nms_thresholds)
        return len(bounding_boxes), bounding_boxes, landmarks

    def face_align(self, img_path, crop_size=(180, 180), min_face_size=20.0,
                   thresholds=[0.6, 0.7, 0.8], nms_thresholds=[0.7, 0.7, 0.7]):
    ...
    def face_no_align(self, img_path, crop_size=(96, 112), min_face_size=20.0,
                   thresholds=[0.6, 0.7, 0.8], nms_thresholds=[0.7, 0.7, 0.7]):
        assert os.path.exists(img_path), "No Such File %s" % img_path
        image = Image.open(img_path)
        face_nums, bounding_boxes, landmarks = self.detect(image,min_face_size=min_face_size,thresholds=thresholds,nms_thresholds=nms_thresholds)
...
if __name__ == '__main__':
    image_file = "F:\\TestImages\\中年男性\\000053.jpg"
    ##init a WutongFace
    wface = WutongFace()
    ...
    faces_align = wface.face_align(image_file,crop_size=(180,180))
    faces_align[0].show()
